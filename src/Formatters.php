...
final class Formatters
{
     ...
     * derived from the class constants so this list cannot drift from them.
     * Any new const whose value starts with "formatter:" is automatically included.
     *
     * @return string[]
     */
    public static function known()
    {
        ...
        foreach ((new \ReflectionClass(static::class))->getConstants() as $value) {
            if (is_string($value) && strpos($value, $prefix) === 0) {
                $names[] = substr($value, strlen($prefix));
            }
        }
        return $names;
    }
